...
func checkMachineDeploymentsHealthy(machineDeployments []machinev1alpha1.MachineDeployment) (bool, *string, error) {
	for _, deployment := range machineDeployments {
		for _, failedMachine := range deployment.Status.FailedMachines {
			return false, pointer.StringPtr("MachineDeploymentHasFailedMachines"), fmt.Errorf("machine %s failed: %s", failedMachine.Name, failedMachine.LastOperation.Description)
		}

		if err := CheckMachineDeployment(&deployment); err != nil {
			return false, pointer.StringPtr("MachineDeploymentUnhealthy"), fmt.Errorf("machine deployment %s in namespace %s is unhealthy: %v", deployment.Name, deployment.Namespace, err)
		}
	}
...
func checkNodesScalingUp(machineList *machinev1alpha1.MachineList, readyNodes, desiredMachines int) (gardencorev1beta1.ConditionStatus, *string, error) {
	...
	if machineObjects := len(machineList.Items); machineObjects < desiredMachines {
		return gardencorev1beta1.ConditionFalse, pointer.StringPtr("MissingMachines"), fmt.Errorf("not enough machine objects created yet (%d/%d)", machineObjects, desiredMachines)
	}

	var pendingMachines, errorneousMachines int
	...
	if errorneousMachines > 0 {
		return gardencorev1beta1.ConditionFalse, pointer.StringPtr("ErrorneousMachines"), fmt.Errorf("%d machine objects are errorneous", errorneousMachines)
	}
	if pendingMachines == 0 {
		return gardencorev1beta1.ConditionFalse, pointer.StringPtr("MissingNodes"), fmt.Errorf("not enough ready worker nodes registered in the cluster (%d/%d)", readyNodes, desiredMachines)
	}

	return gardencorev1beta1.ConditionProgressing, pointer.StringPtr("PendingMachines"), fmt.Errorf("%d machines are being provisioned and should join the cluster and become ready soon", pendingMachines)
}
...
func checkNodesScalingDown(machineList *machinev1alpha1.MachineList, nodeList *corev1.NodeList, registeredNodes, desiredMachines int) (gardencorev1beta1.ConditionStatus, *string, error) {
	...
	for _, node := range nodeList.Items {
		if node.Spec.Unschedulable {
			...
			if !ok {
				return gardencorev1beta1.ConditionFalse, pointer.StringPtr("MachineNotFound"), fmt.Errorf("machine object for cordoned node %s not found", node.Name)
			}
			if machine.DeletionTimestamp == nil {
				return gardencorev1beta1.ConditionFalse, pointer.StringPtr("NodeUnexpectedlyCordoned"), fmt.Errorf("cordoned node %s found but corresponding machine object does not have deletion timestamp", node.Name)
			}
			cordonedNodes++
		}
	}
	...
	if registeredNodes-cordonedNodes != desiredMachines {
		return gardencorev1beta1.ConditionFalse, pointer.StringPtr("TooManyNodes"), fmt.Errorf("too many worker nodes registered, exceeds maximum desired machine count (%d/%d)", registeredNodes, desiredMachines)
	}

	return gardencorev1beta1.ConditionProgressing, pointer.StringPtr("DrainingMachines"), fmt.Errorf("%d machines are being drained and about to being deprovisioned", cordonedNodes)
}
